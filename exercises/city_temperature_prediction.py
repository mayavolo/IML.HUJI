...
if __name__ == '__main__':
    ...
    poly = PolynomialFitting(k)
    israel_train = israel_samples["Day Of Year"].to_numpy()
    jordan_labels = y.loc[jordan_samples.index]

    # error over jordan
    poly.fit(np.array([israel_train]).transpose(), y.loc[israel_samples.index])
    q5_losses.append(poly.loss(np.array([X["Day Of Year"].to_numpy()]).transpose(), y.to_numpy()))

    fig = plt.figure(5)
    months = np.arange(1, 12, 1)
    plt.bar(months, np.array(q5_losses))
    plt.xlabel("Month")
    plt.ylabel("Loss")
