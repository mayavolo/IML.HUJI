...
class RidgeRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        # Returning the closed-form solution of Ridge regression: w=((((A^t)A)+c*I)^-1)(A^t)*b
        self.coefs_ = np.matmul(
            np.matmul(np.linalg.inv(np.matmul(np.transpose(X), X) + self.lam_ * np.eye(X.shape[1])), np.transpose(X)),
            y)
